const fnlog = require('./structs/fnlog')

try {
  var cookieParser = require("cookie-parser");
} catch {
  fnlog.warn('Missing module(s), running npm i')
  ...
  try {
    var cookieParser = require("cookie-parser");
  } catch { fnlog.Error('Module install failed, join our discord for more help: https://dsc.gg/neonite'); }
}
...
axios.get('https://raw.githubusercontent.com/NeoniteDev/NeoniteV2/main/package.json', { validateStatus: undefined }).then((response) => {
  if (response.status == 200) {
    ...
    if (compare > 0) {
      console.log('\n')
      fnlog.warn(`NEW UPDATE IS AVAILABLE, PLEASE CONSIDER UPDATING TO FIX POTENTIAL BUGS AND SECURITY ISSUES.\nVERSION ${response.data.version} IS NOW AVAILABLE\nCURRENT VERSION IS ${version}`, false)
    }
  }
});
...
(function() {
  ...
  app.listen(port, () => {
    ...
    fnlog.Log(`v${version} is up and listening on port ${port || 5595}!`);
  });
